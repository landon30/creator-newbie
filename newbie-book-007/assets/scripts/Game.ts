...
import { _decorator, Collider, Collider2D, Component, Contact2DType, Director, input, Input, instantiate, Label, Node, physics, Prefab, RigidBody, RigidBody2D, Vec2, Vec3 } from 'cc';
...
// 2023.05.22 3D版本 移植2D核心逻辑
// 关于小球位置，挡板位置等可以新建一个测试场景测试
@ccclass('Game')
export class Game extends Component {
    ...
    onTouchStart() {
        ...
        // FIX 修改 from RigidBody2D 2 RigidBody
    ...
    onCollisionHandler() {
        // FIX 修改 from RigidBody2D 2 RigidBody2D
        let rigidBody = this.ballNode.getComponent(RigidBody);
        // FIX 修改 from Collider2D 2 Collider
        let collider = this.ballNode.getComponent(Collider);

        // FIX 修改 from Contact2DType.BEGIN_CONTACT 2 onCollisionEnter
    ...
    public checkBlockOut(blockNode: Node): void {
        ...
        if(blockNode.position.x < -3) {
            ...
            blockNode.position = new Vec3(this.getLastBlockPosx() + this.blockGap,0,0);
